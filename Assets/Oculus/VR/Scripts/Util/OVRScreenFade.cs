...
public class OVRScreenFade : MonoBehaviour
{
	...
	public void FadeIn(float duration, Action onEnd = null)
	{
		...
		_isFaded = false;
		StartCoroutine(Fade( 0.0f, duration, onEnd));
	}
	...
	public void FadeOut(float duration, Action onEnd = null)
	{
		...
		_isFaded = true;
		StartCoroutine(Fade(1, duration, onEnd));
	}
	...
	/// </summary>
	IEnumerator Fade(float endAlpha, float duration, Action onEnd)
	{
		float elapsedTime = 0.0f;
		while (elapsedTime < duration)
		{
			elapsedTime += Time.deltaTime;
			animatedFadeAlpha = Mathf.Lerp(_currentAlpha, endAlpha, Mathf.Clamp01(elapsedTime / duration));
			SetMaterialAlpha();
